...
DROP PROCEDURE IF EXISTS GetUpcomingContractReturns;

DELIMITER //

CREATE PROCEDURE GetUpcomingContractReturns(IN upcoming_days INT)
BEGIN
    SELECT 
        c.contract_id,
    ...
    WHERE 
        c.end_date BETWEEN CURDATE() AND CURDATE() + INTERVAL upcoming_days DAY;
END //
